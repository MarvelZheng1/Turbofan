...
def Compressor_Free_Vortex(rps, r_hub_vec_stages, r_tip_vec_stages, ang_vel, degR_m, rho_m_vec_stages, cp, R, T0_stages, m_dot_target, e_c, gamma):
    ...
    full_vec_length = np.len(r_hub_vec_stages)*2-1

    ## ======== Vector Creation and Initialization ========
    emptyLists = [[] for _ in range(full_vec_length)]
    Ctheta_spans = np.array(emptyLists, dtype=object)
    z_spans      = np.array(emptyLists, dtype=object)
    rho_spans    = np.array(emptyLists, dtype=object)
    T_spans      = np.array(emptyLists, dtype=object)
    r_spans      = np.array(emptyLists, dtype=object)

    r_hub_vec_full = np.ones(full_vec_length)
    r_tip_vec_full = np.ones(1, full_vec_length)
    rho_m_vec_full = np.ones(1, full_vec_length)
