...
% else
stator_params = BDLC_IFN.blade_parameters(S_R, S_R_LE, S_R_TE, S_Cx, S_ttc, S_zeta, S_beta_IN, S_ep_IN, S_beta_OUT, S_ep_OUT, S_N_B, S_blade_height, S_V_mag_mid, "Stator", resolution, iteration_threshold);
% end
% 
...
% else
rotor_params =  BDLC_IFN.blade_parameters(T_R, T_R_LE, T_R_TE, T_Cx, T_ttc, T_zeta, T_beta_IN, T_ep_IN, T_beta_OUT, T_ep_OUT, T_N_B, T_blade_height, T_V_mag_mid, "Rotor", resolution, iteration_threshold);
% end

% Stuff (math)
[stator_blade, mid_profile_index, ~] = BDLC_IFN.generate_blade(stator_params, num_interpolate);
[rotor_blade, ~, ~]  = BDLC_IFN.generate_blade(rotor_params, num_interpolate);
...
nexttile
title("Full Blade")
BDLC_plotSet(rotor_blade, stator_blade, plot_throat, plot_t_max, plot_spline_controls, LE_align, show_curvature, flip_stator, num_stators, num_rotors, 2:length(rotor_blade)-1, mid_profile_index)
% nexttile
% title("Hub Profiles")
% BDLC_plotSet(rotor_blade, stator_blade, plot_throat, plot_t_max, plot_spline_controls, LE_align, show_curvature, flip_stator, num_stators, num_rotors, 2, 2)
% nexttile
% title("Mid Profiles")
% BDLC_plotSet(rotor_blade, stator_blade, plot_throat, plot_t_max, plot_spline_controls, LE_align, show_curvature, flip_stator, num_stators, num_rotors, mid_profile_index, mid_profile_index)
% nexttile
% title("Tip Profiles")
% BDLC_plotSet(rotor_blade, stator_blade, plot_throat, plot_t_max, plot_spline_controls, LE_align, show_curvature, flip_stator, num_stators, num_rotors, length(rotor_blade)-1, length(rotor_blade)-1)

% Export profiles for Solidworks and Ansys
BDLC_export(rotor_blade, stator_blade, LE_align, mid_profile_index, mid_profile_index)
