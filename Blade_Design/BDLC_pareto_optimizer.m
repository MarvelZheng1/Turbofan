...
for i = 1:length(x_inputs)    
    blade_params =  BDLC_IFN.blade_parameters(R, R_LE, R_TE, x_inputs(i,1), x_inputs(i,2), zeta, beta_IN, ep_IN, beta_OUT, ep_OUT, x_inputs(i,3), blade_height, V_mag_mid, "Rotor", resolution, iteration_threshold);
    [blade, ~, success] = BDLC_IFN.generate_blade(blade_params, 0);
    if success == false
        fprintf("huh?!!?!?")
    else
        if i == min_dist_index
            BDLC_plotSingle(blade, type, plot_throat, plot_t_max, plot_spline_controls, LE_align, show_curvature, flip_stator, num_blades, 3, 3, "highlight")
        else
            BDLC_plotSingle(blade, type, plot_throat, plot_t_max, plot_spline_controls, LE_align, show_curvature, flip_stator, num_blades, 3, 3, "regular")
        end
    end
end

figure(3)
blade_params =  BDLC_IFN.blade_parameters(R, R_LE, R_TE, x_inputs(min_dist_index,1), x_inputs(min_dist_index,2), zeta, beta_IN, ep_IN, beta_OUT, ep_OUT, x_inputs(min_dist_index,3), blade_height, V_mag_mid, "Rotor", resolution, iteration_threshold);
[blade, ~, success] = BDLC_IFN.generate_blade(blade_params, 0);
if success == false
    fprintf("huh?!!?!?")
else
...
function F = objectives(x)
    ...
    blade_params =  BDLC_IFN.blade_parameters(R, R_LE, R_TE, x(1), x(2), zeta, beta_IN, ep_IN, beta_OUT, ep_OUT, x(3), blade_height, V_mag_mid, "Rotor", resolution, iteration_threshold);
    [blade, ~, success] = BDLC_IFN.generate_blade(blade_params, 0);
    if success == false
        max_curvature = 10;
        average_zweifel = 10;
