    ...
    methods(Static)
        ...
        function [full_blade, mid_profile_index, success] = generate_blade(params, num_interpolate)
            ...
            for i = 1:num_profiles
                stack(i).beta_IN = BDLC_IFN.rad_eq(stack(i).V_mag_mid, r_mid, params.beta_IN, radii(i));
                stack(i).beta_OUT = BDLC_IFN.rad_eq(stack(i).V_mag_mid, r_mid, params.beta_OUT, radii(i));
                stack(i).R = radii(i);
            end
            ...
            for i = num_profiles:-1:1
                try
                    full_blade(i) = BDLC_pritchard(stack(i));
                catch
                    warning("??????????????")
                    success = false;
                    return
